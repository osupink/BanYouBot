	...
	case 'transfer':
		...
		if (is_numeric($commandArr[0]) && (is_numeric($commandArr[1]) || is_float($commandArr[1]))) {
			if ($commandArr[1] <= 0) {
				...
				if (AddMoneyEvent('Transfer-',$reqQQNumber,"-{$commandArr[1]}") !== 0) {
					if (AddMoneyEvent('Transfer+',$commandArr[0],$commandArr[1]) !== 0) {
						...
						Announce("[BanCoin] {$received_username} 通过转账被赠送了 {$commandArr[1]}.", $commandArr[0]);
					} else {
